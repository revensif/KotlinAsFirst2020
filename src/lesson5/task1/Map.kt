...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((key, value) in grades) {
        result.getOrPut(value) { mutableListOf() }.add(key)
    }
    return result
}
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((key, value) in mapB) {
        val s = result.getOrDefault(key, "")
        if (!result.containsKey(key)) result[key] = value
        if (result[key] != value) result[key] = "$s, $value"
    }
    return result
}
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    ...
    for (i in chars) charSet.add(i.toLowerCase())
    for (letter in word) {
        if (letter.toLowerCase() !in charSet) return false
    }
    return true
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        if (result.containsKey(number - list[i]))
            return Pair(result[number - list[i]]!!, i)
        else if (!result.containsKey(list[i]))
            result[list[i]] = i
    }
    return Pair(-1, -1)
}
